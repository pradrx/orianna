...
// Maybe eventually we will do true algebraic notation, but for now
// lets just take two squares delimited by a space instead for ease
Move Game::parse_input(std::string move)
{
    std::istringstream iss(move);

    std::string curr_pos, new_pos;
    iss >> curr_pos >> new_pos;
}
